function [out1,out2] = traincgp(varargin)
%TRAINCGP Conjugate gradient backpropagation with Polak-Ribiere updates.
%
%  <a href="matlab:doc traincgp">traincgp</a> is a network training function that updates weight and
%  bias values according to the conjugate gradient backpropagation
%   with Polak-Ribiere updates.
%
%  [NET,TR] = <a href="matlab:doc traincgp">traincgp</a>(NET,X,T) takes a network NET, input data X
%  and target data T and returns the network after training it, and a
%  a training record TR.
%
%  [NET,TR] = <a href="matlab:doc traincgp">traincgp</a>(NET,X,T,Xi,Ai,EW) takes additional optional
...
%
%    net.<a href="matlab:doc nnproperty.net_trainFcn">trainFcn</a> = 'traincgp';
...
info = nnfcnTraining(mfilename,'Conjugate Gradient with Polak-Ribiere Restarts',8.0,...
...
if (rem(worker.epoch,worker.num_X)==0) || (worker.norm_sqr==0)
    worker.Z = 0;
else
    worker.Z = ((worker.gX - worker.gX_old)' * worker.gX)/worker.norm_sqr;
end

% Calculate new search direction
worker.dX = -worker.gX + worker.dX_old * worker.Z;

% Save new directions and norm of gradient
worker.gX_old = worker.gX;
worker.dX_old = worker.dX;
worker.norm_sqr = worker.normnew_sqr;
worker.perf_old = worker.perf;
