    ...
    methods
        ...
        function [gWB,trainPerf,trainN] = grad(lib,calcNet)
            ...
            if lib.isMainWorker
                ...
                if(isfield(lib.calcHints.perfParam, 'sparsityRegularization') && strcmp(lib.calcMode.mode,'nnMex'))
                    transferFunction = iGetFirstLayerTransferFunction(lib.calcHints.long);
                    if(iHasUnitOutputRange(transferFunction))
                        [sparsePerf, sparseGrad] = msesparse.computeSparsityRegularization(calcNet, lib.calcMode, ...
...
function transferFunction = iGetFirstLayerTransferFunction(hints)
offset1 = 6 + hints(4);
offset2 = offset1 + 30;
