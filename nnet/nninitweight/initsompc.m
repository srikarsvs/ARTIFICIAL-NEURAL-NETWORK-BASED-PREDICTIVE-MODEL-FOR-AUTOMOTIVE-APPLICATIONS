...
function settings = configure_weight(x)
  ...
  if inputSize == 0
    % No weights - do nothing
  elseif settings.numElements >= 1000000
    % SVD calculation will get too slow, use too much memory
    settings.inputSize = inputSize;
    settings.mean = mean(x,2);
  else
    ind = ~isfinite(x);
    x(ind) = sign(x(ind));
    posMean = mean(x,2);
    x = x - posMean(:,ones(1,numSamples));
...
function w = initialize_input_weight(net,i,j,config)
  ...
  elseif config.numElements >= 1000000
