...
%    <a href="matlab:doc view">view</a>(net)
%
%  <strong>Configuration with Parallel Computing</strong>
%
%  The function <a href="matlab:doc configure">configure</a> will only use the first worker's data if called
%  with <a href="matlab:doc Composite">Composite</a> data.  Call <a href="matlab:doc configure">configure</a> with desired non-Composite data
%  before calling <a href="matlab:doc train">train</a> if other configuration data is desired.
%
%  An error is generated if <a href="matlab:doc configure">configure</a> is called with <a href="matlab:doc gpuArray">gpuArray</a> data.  Call
%  <a href="matlab:doc configure">configure</a> with non-gpuArray data before calling <a href="matlab:doc train">train</a> with <a href="matlab:doc gpuArray">gpuArray</a> data.
%
...
elseif nargin == 2
  ...
  % configure(net,x)
  x = gatherGPUandCompositeData(in2);
...
elseif (nargin == 3) && ~ischar(in2)
  ...
  t = gatherGPUandCompositeData(in3);
  if isa(x,'Composite'), t = t{1}; end
  [t,err] = nntype.data('format',t);
...
elseif nnstring.first_match(lower(in2),{'input','inputs'})

  % configure(net,'inputs',x) -or- (net,'inputs',x,xi)
  x = gatherGPUandCompositeData(in3);
  [x,err] = nntype.data('format',x);
  if ~isempty(err),nnerr.throw(nnerr.value(err,'Input data')); end
...
elseif nnstring.first_match(lower(in2),{'output','outputs','target','targets'})
...
function d = gatherGPUandCompositeData(d)

% Gather worker 1 data for Composites data
if isa(d,'Composite')
  error(message('nnet:configure:CompositeDataUnconfigured'));
end

% Throw error for gpuArray data
if isa(d,'gpuArray')
  error(message('nnet:configure:gpuArrayDataUnconfigured'));
end
if isa(d,'cell')
  for i=1:numel(d)
    if isa(d{i},'gpuArray')
