...
for i=1:numSubscripts
  ...
  case '.'
    ...
    % Single input and output properties
    if strcmpi(subs,'input')
      if (v.numInputs ~= 1)
        nnerr.throw('Args','Property ''input'' is only valid for network''s with exactly one input.')
      else
        v = v.inputs{1};
        moresubs = subscripts(i+1:end);
        if ~isempty(moresubs), v = subsref(v,moresubs); end
        return;
      end
    end
    if strcmpi(subs,'output')
      if (v.numOutputs ~= 1)
        nnerr.throw('Args','Property ''input'' is only valid for network''s with exactly one output.')
      else
        v = v.outputs{v.outputConnect};
        moresubs = subscripts(i+1:end);
        if ~isempty(moresubs), v = subsref(v,moresubs); end
        return;
      end
